...
void RunCudaKernel(const float* p_Input, float* p_Output, int p_Width, int p_Height, int p_LumaMath, int* p_Switch, float* p_Scale)
{
dim3 threads(128, 1, 1);
dim3 blocks(((p_Width + threads.x - 1) / threads.x), p_Height, 1);

VideoGradeKernel<<<blocks, threads>>>(p_Input, p_Output, p_Width, p_Height, p_LumaMath, p_Switch[0], p_Switch[1], p_Switch[2], p_Switch[3], 
p_Scale[0], p_Scale[1], p_Scale[2], p_Scale[3], p_Scale[4], p_Scale[5], p_Scale[6], p_Scale[7], p_Scale[8], p_Scale[9], p_Scale[10], p_Scale[11], 
p_Scale[12], p_Scale[13], p_Scale[14], p_Scale[15], p_Scale[16], p_Scale[17], p_Scale[18], p_Scale[19], p_Scale[20]);
