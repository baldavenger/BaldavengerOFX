...
void Qualifier::processImagesCUDA()
{
...
void QualifierPlugin::changedParam(const OFX::InstanceChangedArgs& p_Args, const std::string& p_ParamName)
{
	...
	if (p_ParamName == "OutputAlpha")
	{
		...
		int outputAlpha_i;
		m_outputAlpha->getValueAtTime(p_Args.time, outputAlpha_i);
    	OutputAlphaEnum outputAlpha = (OutputAlphaEnum)outputAlpha_i;
    	int Output = outputAlpha_i;
...
void QualifierPluginFactory::describeInContext(OFX::ImageEffectDescriptor& p_Desc, OFX::ContextEnum /*p_Context*/)
{
	...
	{
	ChoiceParamDescriptor *param = p_Desc.defineChoiceParam(kParamOutputAlpha);
	...
	assert(param->getNOptions() == (int)eOutputAlphaLuma);
	param->appendOption(kParamOutputAlphaOptionLuma, kParamOutputAlphaOptionLumaHint);
	...
	assert(param->getNOptions() == (int)eOutputAlphaHueLuma);
	param->appendOption(kParamOutputAlphaOptionHueLuma, kParamOutputAlphaOptionHueLumaHint);
	assert(param->getNOptions() == (int)eOutputAlphaSaturationLuma);
	param->appendOption(kParamOutputAlphaOptionSaturationLuma, kParamOutputAlphaOptionSaturationLumaHint);
	assert(param->getNOptions() == (int)eOutputAlphaOff);
	param->appendOption(kParamOutputAlphaOptionOff, kParamOutputAlphaOptionOffHint);
	param->setDefault( (int)eOutputAlphaAll );
	...
	BooleanParamDescriptor* boolParam = p_Desc.defineBooleanParam("InvertAlphaH");
	boolParam->setDefault(false);
	boolParam->setHint("Inverts the Alpha Channel");
	boolParam->setLabel("invert");
	...
	BooleanParamDescriptor* boolParam = p_Desc.defineBooleanParam("InvertAlphaS");
	boolParam->setDefault(false);
	boolParam->setHint("Inverts the Alpha Channel");
	boolParam->setLabel("Invert");
	boolParam->setParent(*sat);

	DoubleParamDescriptor* param = defineScaleParam(p_Desc, "AlphaSA", "high", "High limit of alpha channel", sat);
	page->addChild(*param);
	...
	BooleanParamDescriptor* boolParam = p_Desc.defineBooleanParam("InvertAlphaL");
	boolParam->setDefault(false);
	boolParam->setHint("Inverts the Alpha Channel");
	boolParam->setLabel("invert");
	...
	{
    ChoiceParamDescriptor* param = p_Desc.defineChoiceParam(kParamLuminanceMath);
    ...
    param->setParent(*luma);
    page->addChild(*param);
	}

	DoubleParamDescriptor* param = defineScaleParam(p_Desc, "AlphaLA", "high", "High limit of alpha channel", luma);
	page->addChild(*param);
	...
	DoubleParamDescriptor* param = defineScaleParam(p_Desc, "Black", "clip black", "Clip alpha shadows", clean);
	param->setDefault(0.0f);
	param->setRange(0.0f, 1.0f);
	param->setIncrement(0.001f);
	param->setDisplayRange(0.0f, 1.0f);
	...
	DoubleParamDescriptor* param = defineScaleParam(p_Desc, "Mix", "mix", "Blends new alpha with original alpha", 0);
	param->setDefault(0.0f);
	param->setRange(-1.0f, 1.0f);
	param->setIncrement(0.001f);
	param->setDisplayRange(-1.0f, 1.0f);
    ...
    {
    DoubleParamDescriptor* param = defineScaleParam(p_Desc, "Hue1", "hue", "hue rotate", huesatv);
    ...
    {
    DoubleParamDescriptor* param = defineScaleParam(p_Desc, "Sat1", "sat", "saturation adjust", huesatv);
    param->setDefault(0.0);
    param->setRange(-1.0, 1.0);
    param->setIncrement(0.001);
    param->setDisplayRange(-1.0, 1.0);
    ...
    {
    DoubleParamDescriptor* param = defineScaleParam(p_Desc, "Luma1", "luma", "luma adjust", huesatv);
    param->setDefault(0.0);
    param->setRange(-1.0, 1.0);
    param->setIncrement(0.001);
    param->setDisplayRange(-1.0, 1.0);
