__global__ void SoftClipAdjustKernel(int p_Width, int p_Height, float p_SoftClipA, float p_SoftClipB,
 float p_SoftClipC, float p_SoftClipD, float p_SoftClipE, float p_SoftClipF, float p_SwitchA, float p_SwitchB, float p_SourceA, 
 float p_SourceB, float p_SourceC, const float* p_Input, float* p_Output)
{
   ...
   if ((x < p_Width) && (y < p_Height))
   {
	...
	float sr = p_SourceA == 1.0f ? r : p_SourceB == 1.0f ? cr : mr;
	float sg = p_SourceA == 1.0f ? g : p_SourceB == 1.0f ? cg : mg;
	float sb = p_SourceA == 1.0f ? b : p_SourceB == 1.0f ? cb : mb;
	...
	float SR = p_SourceA == 1.0f ? CR : CR >= 0.0f && CR <= 1.0f ? (CR < 0.0181f ? (CR * 4.5f) : 1.0993f * powf(CR, 0.45f) - (1.0993f - 1.0f)) : CR;
	float SG = p_SourceA == 1.0f ? CG : CG >= 0.0f && CG <= 1.0f ? (CG < 0.0181f ? (CG * 4.5f) : 1.0993f * powf(CG, 0.45f) - (1.0993f - 1.0f)) : CG;
	float SB = p_SourceA == 1.0f ? CB : CB >= 0.0f && CB <= 1.0f ? (CB < 0.0181f ? (CB * 4.5f) : 1.0993f * powf(CB, 0.45f) - (1.0993f - 1.0f)) : CB;
	  
	p_Output[index + 0] = p_SwitchA == 1.0f ? (SR < 1.0f ? 1.0f : SR) - 1.0f : p_SwitchB == 1.0f ? (SR >= 0.0f ? 0.0f : SR + 1.0f) : SR;
	p_Output[index + 1] = p_SwitchA == 1.0f ? (SG < 1.0f ? 1.0f : SG) - 1.0f : p_SwitchB == 1.0f ? (SG >= 0.0f ? 0.0f : SG + 1.0f) : SG;
	p_Output[index + 2] = p_SwitchA == 1.0f ? (SB < 1.0f ? 1.0f : SB) - 1.0f : p_SwitchB == 1.0f ? (SB >= 0.0f ? 0.0f : SB + 1.0f) : SB;
	p_Output[index + 3] = p_Input[index + 3];
	  
   }
}

void RunCudaKernel(int p_Width, int p_Height, float* p_SoftClip, float* p_Switch, float* p_Source, const float* p_Input, float* p_Output)
{
    dim3 threads(128, 1, 1);
    dim3 blocks(((p_Width + threads.x - 1) / threads.x), p_Height, 1);

    SoftClipAdjustKernel<<<blocks, threads>>>(p_Width, p_Height, p_SoftClip[0], p_SoftClip[1], p_SoftClip[2], 
		p_SoftClip[3], p_SoftClip[4], p_SoftClip[5], p_Switch[0], p_Switch[1], p_Source[0], p_Source[1], p_Source[2], p_Input, p_Output);
}
