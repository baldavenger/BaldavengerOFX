...
inline int size_f2( float2 array[])
{
int Size = sizeof(&array)/sizeof(array[0]);
return Size + 1;
}
...
hue = (180.0f/3.14159265358979323846264338327950288f) * atan2f( sqrtf(3.0f) * (rgb.y - rgb.z), 2.0f * rgb.x - rgb.y - rgb.z);
}
if (hue < 0.0f) hue = hue + 360.0f;
return hue;
}

inline float rgb_2_yc( float3 rgb, float ycRadiusWeight = 1.75f)
{
float r = rgb.x;
float g = rgb.y;
float b = rgb.z;
...
inline mat3 calculate_cat_matrix( float2 src_xy, float2 des_xy)
{
...
inline mat3 calc_sat_adjust_matrix( float sat, float3 rgb2Y)
{
float M[3][3];
...
inline float moncurve_f( float x, float gamma, float offs )
{
float y;
...
inline float3 moncurve_f_f3( float3 x, float gamma, float offs)
{
float3 y;
...
inline float3 moncurve_r_f3( float3 y, float gamma, float offs)
{
float3 x;
...
inline float bt1886_f( float V, float gamma, float Lw, float Lb)
{
...
inline float bt1886_r( float L, float gamma, float Lw, float Lb)
{
float a = powf( powf( Lw, 1.0f/gamma) - powf( Lb, 1.0f/gamma), gamma);
float b = powf( Lb, 1.0f/gamma) / ( powf( Lw, 1.0f/gamma) - powf( Lb, 1.0f/gamma));
...
inline float3 bt1886_f_f3( float3 V, float gamma, float Lw, float Lb)
{
float3 L;
...
inline float3 bt1886_r_f3( float3 L, float gamma, float Lw, float Lb)
{
float3 V;
...
inline float smpteRange_to_fullRange( float in)
{
...
inline float fullRange_to_smpteRange( float in)
{
const float REFBLACK = ( 64.0f / 1023.0f);
const float REFWHITE = ( 940.0f / 1023.0f);
...
inline float3 smpteRange_to_fullRange_f3( float3 rgbIn)
{
float3 rgbOut;
...
inline float3 fullRange_to_smpteRange_f3( float3 rgbIn)
{
float3 rgbOut;
...
inline float3 dcdm_decode( float3 XYZp)
{
float3 XYZ;
...
inline float3 dcdm_encode( float3 XYZ)
{
float3 XYZp;
...
inline float ST2084_2_Y( float N )
{
...
inline float Y_2_ST2084( float C )
{
float L = C / pq_C;
...
inline float3 Y_2_ST2084_f3( float3 in)
{
float3 out;
...
inline float3 ST2084_2_Y_f3( float3 in)
{
float3 out;
...
inline float3 ST2084_2_HLG_1000nits_f3( float3 PQ)
{
...
inline float3 HLG_2_ST2084_1000nits_f3( float3 HLG)
{
const float a = 0.17883277f;
const float b = 0.28466892f;
const float c = 0.55991073f;
...
inline SegmentedSplineParams_c5 RRT_PARAMS()
{
...
inline float segmented_spline_c5_fwd( float x)
{
...
inline SegmentedSplineParams_c9 ODT_48nits()
{
...
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, -6.5f) ), 0.02f}, {segmented_spline_c5_fwd( 0.18f ), 4.8f},
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, 6.5f) ), 48.0f}, 0.0f, 0.04f};
return A;
};

inline SegmentedSplineParams_c9 ODT_1000nits()
{
SegmentedSplineParams_c9 A =
...
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, -12.0f) ), 0.0001f}, {segmented_spline_c5_fwd( 0.18f ), 10.0f},
{segmented_spline_c5_fwd( 0.18 * powf(2.0f, 10.0f) ), 1000.0f}, 3.0f, 0.06f};
return A;
};

inline SegmentedSplineParams_c9 ODT_2000nits()
{
SegmentedSplineParams_c9 A =
{{ -4.9706219331f, -3.0293780669f, -2.1262f, -1.5105f, -1.0578f, -0.4668f, 0.11938f, 0.7088134201f, 1.2911865799f, 1.2911865799f },
{ 0.8019952042f, 1.1980047958f, 1.5943f, 1.9973f, 2.3783f, 2.7684f, 3.0515f, 3.2746293562f, 3.3274306351f, 3.3274306351f },
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, -12.0f) ), 0.0001f}, {segmented_spline_c5_fwd( 0.18f ), 10.0f},
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, 11.0f) ), 2000.0f}, 3.0f, 0.12f};
return A;
};

inline SegmentedSplineParams_c9 ODT_4000nits()
{
SegmentedSplineParams_c9 A =
{{ -4.9706219331f, -3.0293780669f, -2.1262f, -1.5105f, -1.0578f, -0.4668f, 0.11938f, 0.7088134201f, 1.2911865799f, 1.2911865799f },
{ 0.7973186613f, 1.2026813387f, 1.6093f, 2.0108f, 2.4148f, 2.8179f, 3.1725f, 3.5344995451f, 3.6696204376f, 3.6696204376f },
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, -12.0f) ), 0.0001f}, {segmented_spline_c5_fwd( 0.18f ), 10.0f},
{segmented_spline_c5_fwd( 0.18f * powf(2.0f, 12.0f) ), 4000.0f}, 3.0f, 0.3f};
return A;
};

inline float segmented_spline_c5_rev( float y)
{
SegmentedSplineParams_c5 C = RRT_PARAMS();
const int N_KNOTS_LOW = 4;
const int N_KNOTS_HIGH = 4;
...
inline float segmented_spline_c9_fwd( float x, SegmentedSplineParams_c9 C)
{
...
inline float segmented_spline_c9_rev( float y, SegmentedSplineParams_c9 C)
{
const int N_KNOTS_LOW = 8;
const int N_KNOTS_HIGH = 8;
const float KNOT_INC_LOW = (log10f(C.midPoint.x) - log10f(C.minPoint.x)) / (N_KNOTS_LOW - 1.0f);
...
inline float3 segmented_spline_c9_rev_f3( float3 rgbPre) {
...
inline float3 segmented_spline_c5_rev_f3( float3 rgbPre) {
float3 rgbPost;
...
inline float3 ACES_to_ACEScc( float3 ACES) {
ACES = max_f3_f( ACES, 0.0f);
float3 lin_AP1 = mult_f3_f33( ACES, AP0_2_AP1_MAT_B);
float3 out;
...
inline float3 ACEScc_to_ACES( float3 ACEScc) {
float3 lin_AP1;
lin_AP1.x = ACEScc_to_lin( ACEScc.x); lin_AP1.y = ACEScc_to_lin( ACEScc.y); lin_AP1.z = ACEScc_to_lin( ACEScc.z);
float3 ACES = mult_f3_f33( lin_AP1, AP1_2_AP0_MAT_B);
return ACES;
}

inline float3 ACES_to_ACEScg( float3 ACES) {
ACES = max_f3_f( ACES, 0.0f);
float3 ACEScg = mult_f3_f33( ACES, AP0_2_AP1_MAT_B);
return ACEScg;
}

inline float3 ACEScg_to_ACES( float3 ACEScg) {
float3 ACES = mult_f3_f33( ACEScg, AP1_2_AP0_MAT_B);
return ACES;
}

inline float ACESproxy_to_lin( float in)
{
...
inline float3 ACESproxy_to_ACES( float3 In)
{
float3 ACESproxy;
...
inline float lin_to_ACESproxy( float in)
{
float StepsPerStop = 50.0f;
float MidCVoffset = 425.0f;
float CVmin = 64.0f;
...
inline float3 ACES_to_ACESproxy( float3 ACES)
{
ACES = max_f3_f( ACES, 0.0f); 
float3 lin_AP1 = mult_f3_f33( ACES, AP0_2_AP1_MAT_B);
float ACESproxy[3];
...
/*
inline float3 ADX_to_ACES( float3 ADX10)
{
float2 LUT_1D[11] = 
{ {-0.19f, -6.0f},
{ 0.01f, -2.721718645f},
...
float3 cdd;
cdd.x = ( adx.x - 95.0f) / 500.0f;
cdd.y = ( adx.y - 95.0f) / 500.0f;
cdd.z = ( adx.z - 95.0f) / 500.0f;
...
*/
inline float3 convertFromLinear( float3 aces) {
...
inline float3 convertToLinear( float3 aces) {
...
inline float3 ADX_to_ACES( float3 aces) {
...
inline float3 ACES_to_ADX( float3 aces) {
...
inline float normalizedLogCToRelativeExposure( float x)
{
if (x > 0.149659f)
...
inline float3 IDT_Alexa_v3_logC_EI800( float3 Alexa)
{
...
inline float3 IDT_Alexa_v3_raw_EI800_CCT6500( float3 In)
{
...
inline float3 IDT_Panasonic_V35( float3 VLog)
{
...
inline float3 IDT_Canon_C100_A_D55( float3 In)
{
float iR, iG, iB;
...
inline float3 IDT_Canon_C100_A_Tng( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C100mk2_A_D55( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C100mk2_A_Tng( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300_A_D55( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300_A_Tng( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C500_B_D55( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C500_B_Tng( float3 In)
{
float iR, iG, iB;
iR = (In.x * 1023.0f - 64.0f) / 876.0f;
iG = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C500_CinemaGamut_A_D55( float3 In)
{
float3 CLogIRE;
...
inline float3 IDT_Canon_C500_CinemaGamut_A_Tng( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog_BT2020_D_D55( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog_BT2020_D_Tng( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog_CinemaGamut_C_D55( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog_CinemaGamut_C_Tng( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog2_BT2020_B_D55( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog2_BT2020_B_Tng( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog3_BT2020_F_D55( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog3_BT2020_F_Tng( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog3_CinemaGamut_E_D55( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Canon_C300mk2_CanonLog3_CinemaGamut_E_Tng( float3 In)
{
float3 CLogIRE;
CLogIRE.x = (In.x * 1023.0f - 64.0f) / 876.0f;
CLogIRE.y = (In.y * 1023.0f - 64.0f) / 876.0f;
...
inline float3 IDT_Sony_SLog1_SGamut( float3 In)
{
...
inline float3 IDT_Sony_SLog2_SGamut_Daylight( float3 In)
{
...
inline float3 IDT_Sony_SLog2_SGamut_Tungsten( float3 In)
{
...
inline float3 IDT_Sony_SLog3_SGamut3( float3 SLog3)
{
...
inline float3 IDT_Sony_SLog3_SGamut3Cine( float3 SLog3)
{
...
inline float Y_2_linCV( float Y, float Ymax, float Ymin)
{
return (Y - Ymin) / (Ymax - Ymin);
}

inline float linCV_2_Y( float linCV, float Ymax, float Ymin)
{
return linCV * (Ymax - Ymin) + Ymin;
}

inline float3 Y_2_linCV_f3( float3 Y, float Ymax, float Ymin)
{
float3 linCV;
...
inline float3 linCV_2_Y_f3( float3 linCV, float Ymax, float Ymin)
{
float3 Y;
...
inline float3 dimSurround_to_darkSurround( float3 linearCV)
{
float3 XYZ = mult_f3_f33( linearCV, AP1_2_XYZ_MAT);
float3 xyY = XYZ_2_xyY(XYZ);
xyY.z = fmaxf( xyY.z, 0.0f);
...
inline float roll_white_fwd( float in, float new_wht, float width)
{
const float x0 = -1.0f;
...
inline float roll_white_rev( float in, float new_wht, float width)
{
const float x0 = -1.0f;
const float x1 = x0 + width;
const float y0 = -new_wht;
...
inline float lookup_ACESmin( float minLum )
{
float2 minTable[2] = { { log10f(MIN_LUM_RRT), MIN_STOP_RRT }, { log10f(MIN_LUM_SDR), MIN_STOP_SDR } };
return 0.18f * powf( 2.0f, interpolate1D( minTable, 2, log10f( minLum)));
}

inline float lookup_ACESmax( float maxLum )
{
float2 maxTable[2] = { { log10f(MAX_LUM_SDR), MAX_STOP_SDR }, { log10f(MAX_LUM_RRT), MAX_STOP_RRT } };
return 0.18f * powf( 2.0f, interpolate1D( maxTable, 2, log10f( maxLum)));
}

inline float5 init_coefsLow( TsPoint TsPointLow, TsPoint TsPointMid)
{
float5 coefsLow;
...
inline float5 init_coefsHigh( TsPoint TsPointMid, TsPoint TsPointMax)
{
float5 coefsHigh;
...
inline float shift( float in, float expShift)
{
return powf(2.0f, (log2f(in) - expShift));
}

inline TsParams init_TsParams( float minLum, float maxLum, float expShift = 0.0f)
{
...
inline float ssts( float x, TsParams C)
{
const int N_KNOTS_LOW = 4;
const int N_KNOTS_HIGH = 4;
...
inline float inv_ssts( float y, TsParams C)
{
const int N_KNOTS_LOW = 4;
const int N_KNOTS_HIGH = 4;
...
inline float3 ssts_f3( float3 x, TsParams C)
{
float3 out;
...
inline float3 inv_ssts_f3( float3 x, TsParams C)
{
float3 out;
...
inline float glow_inv( float ycOut, float glowGainIn, float glowMid)
{
float glowGainOut;
...
inline float cubic_basis_shaper ( float x, float w)
{
...
inline float uncenter_hue( float hueCentered, float centerH)
{
...
inline float3 rrt_sweeteners( float3 in)
{
float3 aces = in;
...
aces = max_f3_f( aces, 0.0f);
float3 rgbPre = mult_f3_f33( aces, AP0_2_AP1_MAT_B);
rgbPre = max_f3_f( rgbPre, 0.0f);
rgbPre = mult_f3_f33( rgbPre, calc_sat_adjust_matrix( RRT_SAT_FACTOR, make_float3(AP1_2_XYZ_MAT.c0.y, AP1_2_XYZ_MAT.c1.y, AP1_2_XYZ_MAT.c2.y)));
return rgbPre;
}

inline float3 inv_rrt_sweeteners( float3 in)
{
float3 rgbPost = in;
rgbPost = mult_f3_f33( rgbPost, invert_f33(calc_sat_adjust_matrix( RRT_SAT_FACTOR, make_float3(AP1_2_XYZ_MAT.c0.y, AP1_2_XYZ_MAT.c1.y, AP1_2_XYZ_MAT.c2.y))));
rgbPost = max_f3_f( rgbPost, 0.0f);
float3 aces = mult_f3_f33( rgbPost, AP1_2_AP0_MAT_B);
aces = max_f3_f( aces, 0.0f);
float hue = rgb_2_hue( aces);
float centeredHue = center_hue( hue, RRT_RED_HUE);
float hueWeight = cubic_basis_shaper( centeredHue, RRT_RED_WIDTH);
...
inline float3 limit_to_primaries( float3 XYZ, Chromaticities LIMITING_PRI)
{
...
inline float3 dark_to_dim( float3 XYZ)
{
float3 xyY = XYZ_2_xyY(XYZ);
xyY.z = fmaxf( xyY.z, 0.0f);
xyY.z = powf( xyY.z, DIM_SURROUND_GAMMA);
...
inline float3 dim_to_dark( float3 XYZ)
{
float3 xyY = XYZ_2_xyY(XYZ);
xyY.z = fmaxf( xyY.z, 0.0f);
xyY.z = powf( xyY.z, 1.0f / DIM_SURROUND_GAMMA);
...
inline float3 invOutputTransform
(
float3 in,
float Y_MIN,
float Y_MID,
...
inline float ACEScct_to_lin( float in)
{
if (in > Y_BRK)
return powf( 2.0f, in * 17.52f - 9.72f);
else
...
inline float3 ACES_to_ACEScct( float3 in)
{
float3 ap1_lin = mult_f3_f33( in, AP0_2_AP1_MAT_B);
float3 acescct;
acescct.x = lin_to_ACEScct( ap1_lin.x); acescct.y = lin_to_ACEScct( ap1_lin.y); acescct.z = lin_to_ACEScct( ap1_lin.z);
return acescct;
}

inline float3 ACEScct_to_ACES( float3 in)
{
float3 ap1_lin;
ap1_lin.x = ACEScct_to_lin( in.x); ap1_lin.y = ACEScct_to_lin( in.y); ap1_lin.z = ACEScct_to_lin( in.z);
return mult_f3_f33( ap1_lin, AP1_2_AP0_MAT_B);
}

inline float3 ASCCDL_inACEScct
(
float3 acesIn,
...
inline float3 gamma_adjust_linear( float3 rgbIn, float GAMMA, float PIVOT = 0.18f)
{
...
inline float3 sat_adjust( float3 rgbIn, float SAT_FACTOR, float3 RGB2Y = make_float3(RGBtoXYZ( REC709_PRI).c0.y, RGBtoXYZ( REC709_PRI).c1.y, RGBtoXYZ( REC709_PRI).c2.y))
{
const mat3 SAT_MAT = calc_sat_adjust_matrix( SAT_FACTOR, RGB2Y);
return mult_f3_f33( rgbIn, SAT_MAT);
}

inline float3 rgb_2_yab( float3 rgb)
{
...
inline float3 yab_2_rgb( float3 yab)
{
...
inline float3 yab_2_ych(float3 yab)
{
float3 ych = yab;
float yo = yab.y * yab.y + yab.z * yab.z;
ych.y = sqrtf(yo);
ych.z = atan2f(yab.z, yab.y) * (180.0f / 3.14159265358979323846264338327950288f);
if (ych.z < 0.0f) ych.z += 360.0f;
return ych;
}

inline float3 ych_2_yab( float3 ych )
{
float3 yab;
yab.x = ych.x;
...
inline float3 rgb_2_ych( float3 rgb)
{
return yab_2_ych( rgb_2_yab( rgb));
}

inline float3 ych_2_rgb( float3 ych)
{
return yab_2_rgb( ych_2_yab( ych));
}

inline float3 scale_C_at_H( float3 rgb, float centerH, float widthH, float percentC)
{
float3 new_rgb = rgb;
...
inline float3 rotate_H_in_H( float3 rgb, float centerH, float widthH, float degreesShift)
{
float3 ych = rgb_2_ych( rgb);
float3 new_ych = ych;
float centeredHue = center_hue( ych.z, centerH);
...
inline float3 scale_C( float3 rgb, float percentC)
{
float3 ych = rgb_2_ych( rgb);
...
inline float3 overlay_f3( float3 a, float3 b)
{
...
inline float3 LMT_PFE( float3 aces)
{
...
inline float3 LMT_Bleach( float3 aces)
{
float3 a, b, blend;
...
inline float3 RRT_cpu( float3 aces)
{
float saturation = rgb_2_saturation( aces);
float ycIn = rgb_2_yc( aces);
float s = sigmoid_shaper( (saturation - 0.4f) / 0.2f);
...
float3 rgbOces = mult_f3_f33( rgbPost, AP1_2_AP0_MAT_B);

return rgbOces;
}

inline float3 InvRRT_cpu( float3 oces)
{
float3 rgbPre = mult_f3_f33( oces, AP0_2_AP1_MAT_B);
float3 rgbPost;
rgbPost.x = segmented_spline_c5_rev( rgbPre.x);
rgbPost.y = segmented_spline_c5_rev( rgbPre.y);
rgbPost.z = segmented_spline_c5_rev( rgbPre.z);
...
inline float3 ODT_Rec709_100nits_dim( float3 oces)
{
...
float3 linearCV;
linearCV.x = Y_2_linCV( rgbPost.x, 48.0f, powf(10.0f, log10f(0.02f)));
linearCV.y = Y_2_linCV( rgbPost.y, 48.0f, powf(10.0f, log10f(0.02f)));
linearCV.z = Y_2_linCV( rgbPost.z, 48.0f, powf(10.0f, log10f(0.02f)));

linearCV = darkSurround_to_dimSurround( linearCV);
linearCV = mult_f3_f33( linearCV, calc_sat_adjust_matrix( ODT_SAT_FACTOR, make_float3(AP1_2_XYZ_MAT.c0.y, AP1_2_XYZ_MAT.c1.y, AP1_2_XYZ_MAT.c2.y)));
float3 XYZ = mult_f3_f33( linearCV, AP1_2_XYZ_MAT);
...
inline float3 ODT_Rec2020_100nits_dim( float3 oces)
{
...
inline float3 ODT_Rec2020_ST2084_1000nits( float3 oces)
{
Chromaticities DISPLAY_PRI = REC2020_PRI;
mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
...
float3 XYZ = mult_f3_f33( rgbPost, AP1_2_XYZ_MAT);
XYZ = mult_f3_f33( XYZ, D60_2_D65_CAT);
...
inline float3 ODT_Rec2020_P3D65limited_100nits_dim( float3 oces)
{
const Chromaticities DISPLAY_PRI = REC2020_PRI;
const mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
...
inline float3 ODT_sRGB_D60sim_100nits_dim( float3 oces)
{
const Chromaticities DISPLAY_PRI = REC709_PRI;
const mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
const float DISPGAMMA = 2.4f;
...
inline float3 ODT_sRGB_100nits_dim( float3 oces)
{
const Chromaticities DISPLAY_PRI = REC709_PRI;
const mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
const float DISPGAMMA = 2.4f;
...
inline float3 ODT_P3DCI_48nits( float3 oces)
{
...
inline float3 ODT_P3DCI_D60sim_48nits( float3 oces)
{
const Chromaticities DISPLAY_PRI = P3DCI_PRI;
const mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB( DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 ODT_P3DCI_D65sim_48nits( float3 oces)
{
const Chromaticities DISPLAY_PRI = P3DCI_PRI;
const mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB( DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 ODT_P3D65_48nits( float3 oces)
{
...
inline float3 ODT_P3D65_D60sim_48nits( float3 oces)
{
const Chromaticities DISPLAY_PRI = P3D65_PRI;
const mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 ODT_DCDM( float3 oces)
{
float3 rgbPre = mult_f3_f33( oces, AP0_2_AP1_MAT_B);
...
inline float3 ODT_DCDM_P3D60limited( float3 oces)
{
...
inline float3 ODT_RGBmonitor_100nits_dim( float3 oces)
{
Chromaticities DISPLAY_PRI = REC709_PRI;
mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
float DISPGAMMA = 2.4f;
...
inline float3 ODT_RGBmonitor_D60sim_100nits_dim( float3 oces)
{
Chromaticities DISPLAY_PRI = REC709_PRI;
mat3 XYZ_2_DISPLAY_PRI_MAT = XYZtoRGB(DISPLAY_PRI);
float DISPGAMMA = 2.4f;
...
inline float3 InvODT_Rec709_100nits_dim( float3 outputCV)
{
Chromaticities DISPLAY_PRI = REC709_PRI;
mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
float DISPGAMMA = 2.4f;
...
XYZ = mult_f3_f33( XYZ, invert_f33( D60_2_D65_CAT));
linearCV = mult_f3_f33( XYZ, XYZ_2_AP1_MAT);
linearCV = mult_f3_f33( linearCV, invert_f33( calc_sat_adjust_matrix( ODT_SAT_FACTOR, make_float3(AP1_2_XYZ_MAT.c0.y, AP1_2_XYZ_MAT.c1.y, AP1_2_XYZ_MAT.c2.y))));
linearCV = dimSurround_to_darkSurround( linearCV);

float3 rgbPre;
rgbPre.x = linCV_2_Y( linearCV.x, 48.0f, powf(10.0f, log10f(0.02f)));
rgbPre.y = linCV_2_Y( linearCV.y, 48.0f, powf(10.0f, log10f(0.02f)));
rgbPre.z = linCV_2_Y( linearCV.z, 48.0f, powf(10.0f, log10f(0.02f)));
...
float3 oces = mult_f3_f33( rgbPost, AP1_2_AP0_MAT_B);

return oces;
}

inline float3 InvODT_Rec709_D60sim_100nits_dim( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = REC709_PRI;
...
inline float3 InvODT_Rec2020_100nits_dim( float3 outputCV)
{
Chromaticities DISPLAY_PRI = REC2020_PRI;
mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
float DISPGAMMA = 2.4f;
...
inline float3 InvODT_Rec2020_ST2084_1000nits( float3 outputCV)
{
Chromaticities DISPLAY_PRI = REC2020_PRI;
mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
...
inline float3 InvODT_sRGB_D60sim_100nits_dim( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = REC709_PRI;
const mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
const float DISPGAMMA = 2.4f;
...
inline float3 InvODT_sRGB_100nits_dim( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = REC709_PRI;
const mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
const float DISPGAMMA = 2.4f;
...
inline float3 InvODT_P3DCI_48nits( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = P3DCI_PRI;
const mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 InvODT_P3DCI_D60sim_48nits( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = P3DCI_PRI;
const mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 InvODT_P3D65_48nits( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = P3D65_PRI;
const mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ( DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 InvODT_P3D65_D60sim_48nits( float3 outputCV)
{
const Chromaticities DISPLAY_PRI = P3D65_PRI;
const mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ( DISPLAY_PRI);
const float DISPGAMMA = 2.6f;
...
inline float3 InvODT_DCDM( float3 outputCV)
{
float3 XYZ = dcdm_decode( outputCV);
float3 linearCV = mult_f3_f33( XYZ, XYZ_2_AP1_MAT);
...
inline float3 InvODT_DCDM_P3D65limited( float3 outputCV)
{
float3 XYZ = dcdm_decode( outputCV);
XYZ = mult_f3_f33( XYZ, invert_f33(D60_2_D65_CAT));
float3 linearCV = mult_f3_f33( XYZ, XYZ_2_AP1_MAT);
...
inline float3 InvODT_RGBmonitor_100nits_dim( float3 outputCV)
{
Chromaticities DISPLAY_PRI = REC709_PRI;
mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
float DISPGAMMA = 2.4f;
...
inline float3 InvODT_RGBmonitor_D60sim_100nits_dim( float3 outputCV)
{
Chromaticities DISPLAY_PRI = REC709_PRI;
mat3 DISPLAY_PRI_2_XYZ_MAT = RGBtoXYZ(DISPLAY_PRI);
float DISPGAMMA = 2.4f;
...
inline float3 RRTODT_Rec2020_1000nits_15nits_HLG( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 1000.0f;
...
inline float3 RRTODT_Rec2020_1000nits_15nits_ST2084( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 1000.0f;
...
inline float3 RRTODT_Rec2020_2000nits_15nits_ST2084( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 2000.0f;
...
inline float3 RRTODT_Rec2020_4000nits_15nits_ST2084( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 4000.0f;
...
inline float3 RRTODT_Rec709_100nits_10nits_BT1886( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 10.0f;
float Y_MAX = 100.0f;
...
inline float3 RRTODT_Rec709_100nits_10nits_sRGB( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 10.0f;
float Y_MAX = 100.0f;
...
inline float3 InvRRTODT_P3D65_108nits_7_2nits_ST2084( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 7.2f;
float Y_MAX = 108.0f;
...
inline float3 InvRRTODT_Rec2020_1000nits_15nits_ST2084( float3 cv)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 1000.0f;
...
inline float3 InvRRTODT_Rec2020_2000nits_15nits_ST2084( float3 cv)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 2000.0f;
...
inline float3 InvRRTODT_Rec2020_4000nits_15nits_ST2084( float3 cv)
{
float Y_MIN = 0.0001f;
float Y_MID = 15.0f;
float Y_MAX = 4000.0f;
...
inline float3 InvRRTODT_Rec709_100nits_10nits_BT1886( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 10.0f;
float Y_MAX = 100.0f;
...
inline float3 InvRRTODT_Rec709_100nits_10nits_sRGB( float3 aces)
{
float Y_MIN = 0.0001f;
float Y_MID = 10.0f;
float Y_MAX = 100.0f;
...
inline float3 IDT_sRGB( float3 rgb) {
float3 aces;
aces = InvODT_sRGB_100nits_dim(rgb);
aces = segmented_spline_c5_rev_f3( aces);
aces = max_f3_f(aces, 0.0f);
return aces;
}

inline float3 IDT_Rec709( float3 rgb) {
float3 aces;
aces = InvODT_Rec709_100nits_dim(rgb);
aces = segmented_spline_c5_rev_f3( aces);
aces = max_f3_f(aces, 0.0f);
return aces;
}
