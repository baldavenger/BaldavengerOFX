...
inline int lin_to_ACESproxy10( float in)
{
...
inline float3 ACES_to_ACESproxy10( float3 ACES)
{
...
inline float ACESproxy10_to_lin( float in)
{
float StepsPerStop = 50.0f;
float MidCVoffset = 425.0f;
//int CVmin = 64;
...
inline float3 ACESproxy10_to_ACES( float3 In)
{
...
inline int lin_to_ACESproxy12( float in)
{
...
inline float3 ACES_to_ACESproxy12( float3 ACES)
{
...
inline float ACESproxy12_to_lin( float in)
{
float StepsPerStop = 200.0f;
float MidCVoffset = 1700.0f;
//int CVmin = 256;
...
inline float3 ACESproxy12_to_ACES( float3 In)
{
