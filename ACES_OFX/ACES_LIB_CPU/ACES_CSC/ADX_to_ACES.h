...
const mat3 CDD_TO_CID = { {0.75573f, 0.05901f, 0.16134f},
    							{0.22197f, 0.96928f, 0.07406f},
    							{0.02230f, -0.02829f, 0.76460f} };

const mat3 EXP_TO_ACES = { {0.72286f, 0.11923f, 0.01427f},
    							{0.12630f, 0.76418f, 0.08213f},
    							{0.15084f, 0.11659f, 0.90359f} };

float2 LUT_1D[11] = { {-0.190000000000000, -6.000000000000000},
...
inline float3 ADX10_to_ACES( float3 ADX10)
{
...
inline float3 ADX16_to_ACES( float3 ADX16)
{
    // Prepare input values based on application bit depth handling
    float3 adx;
    ...
    float3 cdd;
    cdd.x = ( adx.x - 1520.0f) / 8000.0f;
    cdd.y = ( adx.y - 1520.0f) / 8000.0f;
    cdd.z = ( adx.z - 1520.0f) / 8000.0f;
