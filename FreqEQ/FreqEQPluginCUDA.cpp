...
extern void RunCudaKernel(float* p_Input, float* p_Output, int p_Width, int p_Height, float* p_EQ, int p_Switch, int p_Grey);

void FrequencyEQ::processImagesCUDA()
{
...
RunCudaKernel(input, output, width, height, _eq, _switch, _grey);
}
...
ImageEffect* FrequencyEQPluginFactory::createInstance(OfxImageEffectHandle p_Handle, ContextEnum )
{
