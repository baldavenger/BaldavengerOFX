#include "FrequencyCudaKernel.cuh"

cudaError_t cudaError;

void RunCudaKernel(float* p_Input, float* p_Output, int p_Width, int p_Height, int p_Space, float* p_Blur, float* p_Sharpen, int* p_Switch)
{   	
	int nthreads = 128;
	float* tempBuffer;
	...
	SimpleKernel<<<blocks, threadsT>>>(p_Width, p_Height, p_Input + 0, p_Output + 0);
	}
	if (p_Blur[1] > 0.0f) {
	...
	SimpleKernel<<<blocks, threadsT>>>(p_Width, p_Height, p_Input + 1, p_Output + 1);
	}
	if (p_Blur[2] > 0.0f) {
	...
	SimpleKernel<<<blocks, threadsT>>>(p_Width, p_Height, p_Input + 2, p_Output + 2);
	}
	...
	for(int c = 0; c < 3; c++) {
	FrequencySharpen<<<blocks, threadsT>>>(p_Width, p_Height, p_Input + c, p_Output + c, p_Sharpen[c], p_Switch[2]);
	}
    ...
    FrequencySharpen<<<blocks, threadsT>>>(p_Width, p_Height, p_Output + 3, p_Output, p_Sharpen[0], p_Switch[2]);
	...
	FrequencyAdd<<<blocks, threadsT>>>(p_Width, p_Height, p_Output + 3, p_Output);
