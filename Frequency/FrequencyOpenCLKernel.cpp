...
"float fx = xyzR >= 0.008856f ? pow(xyzR, (float)1 / 3) : 7.787f * xyzR + 16.0f / 116; \n" \
"float fy = xyzG >= 0.008856f ? pow(xyzG, (float)1 / 3) : 7.787f * xyzG + 16.0f / 116; \n" \
"float fz = xyzB >= 0.008856f ? pow(xyzB, (float)1 / 3) : 7.787f * xyzB + 16.0f / 116;     \n" \
"float L = (116 * fy - 16) / 100.0f; \n" \
"od[index + 0] = L; \n" \
"od[index + 1] = (500 * (fx - fy)) / 200.0f + 0.5f; \n" \
"od[index + 2] = (200 * (fy - fz)) / 200.0f + 0.5f; \n" \
"od[index + 3] = L; \n" \
"} \n" \
"} \n" \
...
"} \n" \
"} \n" \
"__kernel void k_freqSharpen(__global float *id, __global float *od, int w, int h, float sharpen, int display, int c) \n" \
"{ \n" \
"const int x = get_global_id(0); \n" \
"const int y = get_global_id(1);	 \n" \
...
"} \n" \
"} \n" \
"__kernel void k_freqAdd(__global float *id, __global float *od, int w, int h, int c, int cc) \n" \
"{ \n" \
"const int x = get_global_id(0); \n" \
"const int y = get_global_id(1);				    \n" \
"if ((x < w) && (y < h)) \n" \
"{   \n" \
"const int index = (y * w + x) * 4;																									 \n" \
"od[index + c] = id[index + cc] + od[index + c]; \n" \
"} \n" \
"} \n" \
"__kernel void k_freqSharpenLuma(__global float *id, __global float *od, int w, int h, float sharpen, int display) \n" \
"{ \n" \
"const int x = get_global_id(0); \n" \
"const int y = get_global_id(1);	 \n" \
"float offset = display == 1 ? 0.5f : 0.0f; \n" \
...
"{   \n" \
"const int index = (y * w + x) * 4;										 \n" \
"id[index + 3] = (id[index + 3] - od[index + 0]) * sharpen + offset; \n" \
"if (display == 1) { \n" \
"od[index + 0] = id[index + 3]; \n" \
"od[index + 1] = id[index + 3]; \n" \
"od[index + 2] = id[index + 3]; \n" \
"} \n" \
"} \n" \
"} \n" \
"\n";
...
void RunOpenCLKernel(void* p_CmdQ, float* p_Input, float* p_Output, int p_Width, int p_Height, int p_Space, float* p_Blur, float* p_Sharpen, int* p_Switch)
{
	...
	error |= clSetKernelArg(Simple, 4, sizeof(int), &red);
	...
	error |= clSetKernelArg(Simple, 4, sizeof(int), &green);
	clEnqueueNDRangeKernel(cmdQ, Simple, 2, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
	}
	...
	error |= clSetKernelArg(Simple, 4, sizeof(int), &blue);
	clEnqueueNDRangeKernel(cmdQ, Simple, 2, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
	}
	...
	error |= clSetKernelArg(FreqSharpen, 5, sizeof(int), &p_Switch[2]);
	CheckError(error, "Unable to set kernel arguments");
	...
	error |= clSetKernelArg(FreqAdd, 5, sizeof(int), &c);
	...
	error = clSetKernelArg(FreqSharpenLuma, 0, sizeof(cl_mem), &p_Output);
	...
	error |= clSetKernelArg(FreqSharpenLuma, 5, sizeof(int), &p_Switch[2]);
	CheckError(error, "Unable to set kernel arguments");
	...
	error = clSetKernelArg(FreqAdd, 0, sizeof(cl_mem), &p_Output);
	error |= clSetKernelArg(FreqAdd, 1, sizeof(cl_mem), &p_Output);
	error |= clSetKernelArg(FreqAdd, 2, sizeof(int), &p_Width);
	error |= clSetKernelArg(FreqAdd, 3, sizeof(int), &p_Height);
	error |= clSetKernelArg(FreqAdd, 4, sizeof(int), &red);
	error |= clSetKernelArg(FreqAdd, 5, sizeof(int), &alph);
	CheckError(error, "Unable to set kernel arguments");
	clEnqueueNDRangeKernel(cmdQ, FreqAdd, 2, NULL, globalWorkSize, localWorkSize, 0, NULL, NULL);
