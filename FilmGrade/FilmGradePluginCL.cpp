...
public:
    ...
    float p_DisplayA, float p_DisplayB);
    ...
    for (int y = p_ProcWindow.y1; y < p_ProcWindow.y2; ++y)
    {
        ...
        for (int x = p_ProcWindow.x1; x < p_ProcWindow.x2; ++x)
        {
            ...
            if (srcPix)
            {
                  ...
                  float outR = _display[0] == 1.0f && _display[1] == 1.0f ? (overlayR == 0.0f ? satR : overlayR) : _display[0] == 1.0f ? overlayR : satR;
                  float outG = _display[0] == 1.0f && _display[1] == 1.0f ? (overlayG == 0.0f ? satG : overlayG) : _display[0] == 1.0f ? overlayG : satG;
                  float outB = _display[0] == 1.0f && _display[1] == 1.0f ? (overlayB == 0.0f ? satB : overlayB) : _display[0] == 1.0f ? overlayB : satB;
...
float p_DisplayA, float p_DisplayB)
{
    ...
    _pivot[1] = p_HighP;
    _pivot[2] = p_ContP;
    _display[0] = p_DisplayA;
    _display[1] = p_DisplayB;
}
...
bool FilmGradePlugin::isIdentity(const OFX::IsIdentityArguments& p_Args, OFX::Clip*& p_IdentityClip, double& p_IdentityTime)
{
    ...
    bool aDisplay = m_DisplayA->getValueAtTime(p_Args.time);
...
void FilmGradePlugin::setupAndProcess(ImageScaler& p_ImageScaler, const OFX::RenderArguments& p_Args)
{
    ...
    bool aDisplay = m_DisplayA->getValueAtTime(p_Args.time);
    float displayA = (aDisplay) ? 1.0f : 0.0f;
    
    bool bDisplay = m_DisplayB->getValueAtTime(p_Args.time);
    float displayB = (bDisplay) ? 1.0f : 0.0f; 
    ...
    shadR, shadG, shadB, midR, midG, midB, highR, highG, highB, shadP, highP, contP, displayA, displayB);
...
void FilmGradePluginFactory::describeInContext(OFX::ImageEffectDescriptor& p_Desc, OFX::ContextEnum /*p_Context*/)
{
    ...
    BooleanParamDescriptor* boolParam = p_Desc.defineBooleanParam("displayA");
