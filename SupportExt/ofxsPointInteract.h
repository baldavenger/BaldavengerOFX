...
/*
 * OFX generic point interact with 4 corner points + center point and 4 mid-points.
 * You can use it to define any point in an image resizable by the user.
 */

#ifndef openfx_supportext_ofxsPointInteract_h
#define openfx_supportext_ofxsPointInteract_h
...
#define kParamPointInteractEnable "pointInteractEnable"

#define kParamPointInteractBtmLeft1 "bottomLeft"
#define kParamPointInteractBtmLeftLabel1 "Bottom Left"
#define kParamPointInteractBtmLeftHint1 "Coordinates of the bottom left corner of the point"

#define kParamPointInteractBtmLeft2 "bottomLeft"
#define kParamPointInteractBtmLeftLabel2 "Bottom Left"
#define kParamPointInteractBtmLeftHint2 "Coordinates of the bottom left corner of the point"

#define kParamPointInteractSize "size"
#define kParamPointInteractSizeLabel "Size"
#define kParamPointInteractSizeHint "Width and height of the point"
#define kParamPointInteractSizeDim1 "w"
#define kParamPointInteractSizeDim2 "h"

#define kParamPointInteractInteractive "interactive"
#define kParamPointInteractInteractiveLabel "Interactive Update"
#define kParamPointInteractInteractiveHint "If checked, update the parameter values during interaction with the image viewer, else update the values when pen is released."
...
public:
    ...
    PointInteract(OfxInteractHandle handle,
        ...
        , _size(0)
    {
        if ( _effect->paramExists(kParamPointInteractEnable) ) {
            _enable = effect->fetchBooleanParam(kParamPointInteractEnable);
        }
        _btmLeft1 = effect->fetchDouble2DParam(kParamPointInteractBtmLeft1);
        _btmLeft2 = effect->fetchDouble2DParam(kParamPointInteractBtmLeft2);
        _size = effect->fetchDouble2DParam(kParamPointInteractSize);
        addParamToSlaveTo(_btmLeft1);
        addParamToSlaveTo(_btmLeft2);
        addParamToSlaveTo(_size);
        assert(_btmLeft && _size);
        _interactive = effect->paramExists(kParamPointInteractInteractive) ? effect->fetchBooleanParam(kParamPointInteractInteractive) : 0;
...
class PointOverlayDescriptor
    : public OFX::DefaultEffectOverlayDescriptor<PointOverlayDescriptor, PointInteract>
{
};
} // namespace OFX

#endif /* defined(openfx_supportext_ofxsPointInteract_h) */
