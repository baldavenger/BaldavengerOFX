...
drawPoint(const OfxRGBColourD &color,
          ...
          double x,
          double y,
          PointInteract::DrawStateEnum id,
          PointInteract::DrawStateEnum ds,
          bool keepAR,
          int l)
{
    if (draw) {
...
bool
PointInteract::draw(const OFX::DrawArgs &args)
{
...
bool
PointInteract::penMotion(const OFX::PenArgs &args)
{
    if ( _btmLeft->getIsSecret() || _size->getIsSecret() ||
         !_btmLeft->getIsEnable() || !_size->getIsEnable() ||
         ( _enable && !_enable->getValueAtTime(args.time) ) ) {
    ...
    ///forbid 0 pixels wide crop points
    if (_sizeDrag.x < 1) {
        _sizeDrag.x = 1;
        valuesChanged = true;
    }
...
bool
PointInteract::penDown(const OFX::PenArgs &args)
{
    if ( _btmLeft->getIsSecret() || _size->getIsSecret() ||
         !_btmLeft->getIsEnable() || !_size->getIsEnable() ||
         ( _enable && !_enable->getValueAtTime(args.time) ) ) {
...
bool
PointInteract::penUp(const OFX::PenArgs &args)
{
    if ( _btmLeft->getIsSecret() || _size->getIsSecret() ||
         !_btmLeft->getIsEnable() || !_size->getIsEnable() ||
         ( _enable && !_enable->getValueAtTime(args.time) ) ) {
...
bool
PointInteract::keyDown(const OFX::KeyArgs &args)
{
    if ( _btmLeft->getIsSecret() || _size->getIsSecret() ||
         !_btmLeft->getIsEnable() || !_size->getIsEnable() ||
         ( _enable && !_enable->getValueAtTime(args.time) ) ) {
...
bool
PointInteract::keyUp(const OFX::KeyArgs &args)
{
    if ( _btmLeft->getIsSecret() || _size->getIsSecret() ||
         !_btmLeft->getIsEnable() || !_size->getIsEnable() ||
         ( _enable && !_enable->getValueAtTime(args.time) ) ) {
...
void
PointInteract::loseFocus(const OFX::FocusArgs & /*args*/)
{
...
OfxPointD
PointInteract::getBtmLeft(OfxTime time) const
{
    OfxPointD ret;
...
void
PointInteract::setValue(OfxPointD btmLeft,
                            OfxPointD size,
                            const OfxPointD &pscale)
{
    ...
    _effect->beginEditBlock("setPoint");
