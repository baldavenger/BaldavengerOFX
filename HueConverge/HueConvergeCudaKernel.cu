...
{	
	...
	case 1:
	{
	median3x3(p_In, p_Out, p_Width, x, y);
	}
	break;
	case 2:
	{
	median5x5(p_In, p_Out, p_Width, x, y);
	}
	}
	}
}
...
{	
    ...
    p_In[index + 0] = p_SwitchHue == 1 ? new_ych.x : p_In[index + 0];
...
float *p_Hue1, float *p_Hue2, float *p_Hue3, int p_Display, float *p_Blur, int p_Math, int p_HueMedian, int p_Isolate)
{
	...
	{
	HueMedian<<<blocks, threadsT>>>(p_Output, tempBuffer, p_Width, p_Height, p_HueMedian);
