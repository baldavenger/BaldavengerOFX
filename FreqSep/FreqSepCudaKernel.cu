...
for(int c = 0; c < 4; c++) {
SimpleKernel<<<blocks, threadsT>>>(p_Input + c, p_Output + c, p_Width, p_Height);
}
...
for(int c = 0; c < 3; c++) {
FrequencySharpen<<<blocks, threadsT>>>(p_Input + c, p_Output + c, p_Width, p_Height, p_Sharpen[c], p_Display);
}
if (p_Display != 1) {
if (p_Switch[0] == 1)
...
for(int c = 0; c < 3; c++) {
LowFreqCont<<<blocks, threadsT>>>(p_Output + c, p_Width, p_Height, p_Cont[0], p_Cont[1], p_Switch[2], p_Display);
}
if (p_Display == 0)
for(int c = 0; c < 3; c++) {
FrequencyAdd<<<blocks, threadsT>>>(p_Input + c, p_Output + c, p_Width, p_Height);
}}}
...
if(p_Space == 2)
d_arri_to_lab<<<blocks, threadsT>>>(p_Input, p_Output, p_Width, p_Height);
if(p_Space == 3)
d_acescct_to_lab<<<blocks, threadsT>>>(p_Input, p_Output, p_Width, p_Height);
if (p_Blur[0] > 0.0f) {
...
FrequencySharpenLuma<<<blocks, threadsT>>>(p_Input, p_Output, p_Width, p_Height, p_Sharpen[0], p_Display);
if (p_Display != 1) {
if (p_Switch[1] == 1)
p_Blur[5] = p_Blur[4];
if (p_Blur[3] > 0.0f) {
...
LowFreqContLuma<<<blocks, threadsT>>>(p_Output, p_Width, p_Height, p_Cont[0], p_Cont[1], p_Switch[2], p_Display);
if (p_Display == 0) {
FrequencyAdd<<<blocks, threadsT>>>(p_Input, p_Output, p_Width, p_Height);
if(p_Space == 1)
d_lab_to_rec709<<<blocks, threadsT>>>(p_Output, p_Width, p_Height);
if(p_Space == 2)
d_lab_to_arri<<<blocks, threadsT>>>(p_Output, p_Width, p_Height);
if(p_Space == 3)
d_lab_to_acescct<<<blocks, threadsT>>>(p_Output, p_Width, p_Height);
}}}
